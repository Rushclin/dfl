...
def check_args(args):
    ...
    if args.lr_decay_step > args.R:
        err = f'Learning rate decay step (`{args.lr_decay_step}`) greater than the number of rounds (`{args.R}`)'
        logger.exception(err)
        raise AssertionError(err)
    ...
    logger.info('[CONFIG] Configuration list...')
    for arg in vars(args):
        if 'glove_emb' in str(arg):
            if getattr(args, arg) is True:
                logger.info(f'[CONFIG] - {str(arg).upper()}: ENABLED!')
            else:
                logger.info(f'[CONFIG] - {str(arg).upper()}: DISABLED!')
            continue
